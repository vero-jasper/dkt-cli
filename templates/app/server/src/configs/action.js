  ...
  defaultAction: {
    ...
    async beforeHandler(ctx) {
      ...
      if (this.needLogin === 'maybe') {
        const user = (ctx.request.user = ctx.user = await ctx.request.checkLogin(
          { redirectLogin: false },
        ));
        return;
      }
      ...
      // 如果没有登陆自动跳转登陆页面
      if (this.needLogin) {
        console.log(ctx.request.checkLogin);
        const user = (ctx.request.user = ctx.user = await ctx.request.checkLogin());
        if (!user) {
          return false;
        } else {
          const lj_cookie = ctx.cookies.get(loginConfig.general.tokenName);
          global.lj_cookie = lj_cookie;
        }
      }
